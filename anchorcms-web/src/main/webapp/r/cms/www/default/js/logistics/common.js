...
var HD ={
    ...
    findAttachment:function(id)
    {
    	var config = {
			...
			height:490,
			title:'附件列表',
			url:ctxPath+'/sys/adjunct/findAdjs.html?oid='+id
		};
		ui.createWind(config);	 
    },
    //表单序列化json处理  formobj表单对象
	jsonobj:function(formobj){
	 ...
	 * @return {TypeName} 
	 */
    //表单序列化成Json格式  formobj表单对象
	form2Jsonobj:function(formobj){
		var obj=formobj.serializeArray();
		var resultobj="{";
		if(obj.length>0){
			$.each(obj,function(i,item){
