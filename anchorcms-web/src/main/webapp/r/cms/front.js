 ...
 * 提交留言
 */
Cms.consult = function(callback, form) {
	form = form || "consultForm";
	$("#" + form).validate( {
		submitHandler : function(form) {
			$(form).ajaxSubmit( {
				"success" : callback,
...
Cms.loginCsi2 = function(base, c, options) {
    c = c || "loginCsiDiv";
    $("#" + c).load(base + "/login_csi2.jspx", options);
}
/**
 * 向上滚动js类
 */
 ...
 * 02-27 加载评论列表
 */
Cms.icloudCommentList = function(base, c, options) {
	c = c || "commentList";
	$("#" + c).load(base + "/icloudCommentList.jspx", options);
}
/**
 * 02-27 加载留言列表
 */
Cms.icloudConsultList = function(base, c, options) {
	c = c || "consultList";
	$("#" + c).load(base + "/icloudConsultList.jspx", options);
}
