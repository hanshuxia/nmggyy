...
public class CmsModelItemController {
	private static final Logger log = LoggerFactory
			.getLogger(CmsModelItemController.class);
	...
	public String priority(Integer[] wids, Integer[] priority, String[] label,
						   Boolean[] isSingle, Boolean[] isDisplay, Integer modelId,
						   Boolean isChannel, HttpServletRequest request, ModelMap model) {
		if (wids != null && wids.length > 0) {
			manager.updatePriority(wids, priority, label, isSingle, isDisplay);
		}
	...
	public String save(CmsModelItem bean, Integer modelId, Boolean isChannel,
		...
		log.info("update CmsModelItem id={}.", bean.getModelitemId());
		model.addAttribute("modelId", bean.getModel().getModelId());
	...
	public String update(CmsModelItem bean, HttpServletRequest request,
			ModelMap model) {
		WebErrors errors = validateUpdate(bean.getModelitemId(), bean, request);
		if (errors.hasErrors()) {
			return errors.showErrorPage(model);
		}
	...
	public String delete(Integer[] ids, Integer modelId, Boolean isChannel,
		...
		for (CmsModelItem bean : beans) {
			log.info("delete CmsModelItem id={}", bean.getModelitemId());
		}
		model.addAttribute("modelId", modelId);
		model.addAttribute("isChannel", isChannel);
		return "redirect:v_list.do";
