...
import static com.anchorcms.common.constants.Constants.TPLDIR_MESSAGE;
...
public class MessageController {
	...
	public static final String SENDMESSAGE_ADD = "tpl.sendMessageAdd";
	...
	public static final String MESSAGE_FORWARD_ADD = "tpl.forwardMessageAdd";
	...
	public String message_inbox(Integer pageNo, String title,
		...
		if (box.equals(0)) {
			// 收件箱
			pagination = receiverMessageMng.getPage(site.getSiteId(), null, user
			...
			// 发件箱
			pagination = messageMng.getPage(site.getSiteId(), user.getUserId(), null,
			...
			// 垃圾箱(可能从收件箱或者从发件箱转过来)
			pagination = receiverMessageMng.getPage(site.getSiteId(), user.getUserId(),
     ...
     * @return
     */
	@RequestMapping(value = "/member/sendMessage_add.jspx")
	public String sendMessage_add(HttpServletRequest request,Integer channelId,String nextUrl,
							  HttpServletResponse response,String userName,String cyrh, ModelMap model) {
		CmsSite site = CmsUtils.getSite(request);
		CmsUser user = CmsUtils.getUser(request);
		FrontUtils.frontData(request, model, site);
		MemberConfig mcfg = site.getConfig().getMemberConfig();
		...
		model.addAttribute("channel",channelMng.findById(channelId));
		model.addAttribute("nextUrl",nextUrl);
		model.addAttribute("cyrh",cyrh);
		return FrontUtils.getTplPath(request, site.getSolutionPath(),
				TPLDIR_MESSAGE, SENDMESSAGE_ADD);
	}
	...
	public String message_update(CmsMessage message, String nextUrl,
								 ...
								 HttpServletResponse response, ModelMap model,Integer id,String msgTitle,String msgContent) {
		CmsSite site = CmsUtils.getSite(request);
		CmsUser user = CmsUtils.getUser(request);
		FrontUtils.frontData(request, model, site);
		MemberConfig mcfg = site.getConfig().getMemberConfig();
		...
		CmsMessage messages =dao.findById(id);
		messages.setMsgTitle(msgTitle);
		messages.setMsgContent(msgContent);
		messages.setMsgBox(2);
		message = messageMng.update(messages);
		// 更新发送表的信息，收件表的信息同步更新
		Set<CmsReceiverMessage> receiverMessageSet = message.getReceiverMsgs();
		Iterator<CmsReceiverMessage> it = receiverMessageSet.iterator();
		CmsReceiverMessage receiverMessage;
     ...
     * @return
     */
	@RequestMapping(value = "/member/message_sendRead.jspx")
	public String message_sendRead(Integer id, Integer box,
							   HttpServletRequest request, HttpServletResponse response,
							   ModelMap model) {
		CmsSite site = CmsUtils.getSite(request);
		CmsUser user = CmsUtils.getUser(request);
	...
	public String message_forward(Integer id, HttpServletRequest request,
		...
		return FrontUtils.getTplPath(request, site.getSolutionPath(),
				TPLDIR_MESSAGE, MESSAGE_FORWARD_ADD);
	}

	// 发件箱和草稿箱的转发
	@RequestMapping(value = "/member/message_sendForward.jspx")
	public String message_sendForward(Integer id, HttpServletRequest request,
								  HttpServletResponse response, ModelMap model) {
		CmsSite site = CmsUtils.getSite(request);
		CmsUser user = CmsUtils.getUser(request);
		FrontUtils.frontData(request, model, site);
		...
		if (user == null) {
			...
			for (Integer i = 0; i < ids.length; i++) {
				...
				if (receiverMessage != null
					...
					if (receiverMessage.getMsgBox() == 3) {
						// 草稿直接删除
						receiverMessage.setMessage(null);
						if (message != null) {
