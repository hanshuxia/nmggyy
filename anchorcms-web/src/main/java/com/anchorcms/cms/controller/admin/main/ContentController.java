...
import static com.anchorcms.common.constants.Constants.EXT_NAME_IN_REQUEST;
import static com.anchorcms.common.constants.Constants.EXT_PATH_IN_REQUEST;
...
public class ContentController {
	...
	public String add(Integer cid,Integer modelId, HttpServletRequest request,ModelMap model) {
		...
		// 模型项列表
		List<CmsModelItem> itemList = cmsModelItemMng.getList(m.getModelId(), false,
				false);
		// 栏目列表
		...
		if (c != null) {
			topicList = cmsTopicMng.getListByChannel(c.getChannelId());
		} else {
		...
		Object jxdExtValue =request.getAttribute(EXT_NAME_IN_REQUEST);
		if(jxdExtValue != null && jxdExtValue != ""){
			model.addAttribute(EXT_NAME_IN_REQUEST,jxdExtValue.toString());
		}
		Object jxdExtPathValue =request.getAttribute(EXT_PATH_IN_REQUEST);
		if(jxdExtPathValue != null && jxdExtPathValue != ""){
			model.addAttribute(EXT_PATH_IN_REQUEST,jxdExtPathValue.toString());
		}
		return "content/add";
	}
	...
	public String save(Content bean, ContentExt ext, ContentTxt txt,
		...
		cmsLogMng.operating(request, "content.log.save", "id=" + bean.getContentId()
				+ ";title=" + bean.getTitle());
		if (cid != null) {
			model.addAttribute("cid", cid);
		}
	...
	public String update(String queryStatus, Integer queryTypeId,
		...
		cmsLogMng.operating(request, "content.log.update", "id=" + bean.getContentId()
				+ ";title=" + bean.getTitle());
	...
	public String uploadAttachment(
		...
		try {
			...
			cmsUserMng.updateUploadSize(user.getUserId(), Integer.parseInt(String.valueOf(file.getSize()/1024)));
	...
	public String importDocs(HttpServletRequest request,ModelMap model) {
		...
		List<Channel> topList = channelMng.getTopListByRigth(user.getUserId(), siteId, true);
		channelList = Channel.getListForSelect(topList, rights, true);
		CmsModel m;
		m = cmsModelMng.getDefModel();
		// TODO m==null给出错误提示
