...
import com.anchorcms.cms.service.assist.CmsAccountDrawMng;
import com.anchorcms.cms.service.assist.CmsAccountPayMng;
...
@Controller
public class CmsAccountPayController {
	private static final Logger log = LoggerFactory.getLogger(CmsAccountPayController.class);
			...
			if(errors.hasErrors()){
				...
				if(StringUtils.isBlank(getWeixinPayUrl())){
					setWeixinPayUrl(PropertyUtils.getPropertyValue(
							new File(realPathResolver.get(com.anchorcms.common.constants.Constants.ANCHORCMS_CONFIG)),WEIXIN_PAY_URL));
				}
				String status=accountPayMng.weixinTransferPay(getWeixinPayUrl(), drawId, bean.getCmsUser(), user, bean.getApplyAmount(),
