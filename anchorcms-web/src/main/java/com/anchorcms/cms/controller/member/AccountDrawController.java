...
public class AccountDrawController {
	...
	public String drawList(Integer pageNo,HttpServletRequest request,
		...
		Pagination pagination=accountDrawMng.getPage(user.getUserId(),null,null,null,
	...
	public String drawInput(HttpServletRequest request,
		...
		Double appliedSum=accountDrawMng.getAppliedSum(user.getUserId());
		model.addAttribute("userAccount", user.getUserAccount());
	...
	private boolean vldOpt(WebErrors errors, CmsSite site, CmsUser user,
		...
		for (Integer id : ids) {
			...
			// 非本用户数据
		/*	if (!d.getDrawUser().getId().equals(user.getUserId())) {
