...
public class CmsModelController {
	private static final Logger log = LoggerFactory
			.getLogger(CmsModelController.class);
	...
	public String save(CmsModel bean, HttpServletRequest request, ModelMap model) {
		...
		log.info("save CmsModel id={}", bean.getModelId());
		cmsLogMng.operating(request, "cmsModel.log.save", "id=" + bean.getModelId()
	...
	public String update(CmsModel bean, HttpServletRequest request,
			ModelMap model) {
		WebErrors errors = validateUpdate(bean.getModelId(), request);
		if (errors.hasErrors()) {
			return errors.showErrorPage(model);
		}
		if(!bean.getIsGlobal()){
		...
		log.info("update CmsModel id={}.", bean.getModelId());
		cmsLogMng.operating(request, "cmsModel.log.update", "id="
				+ bean.getModelId() + ";name=" + bean.getModelName());
	...
	public String delete(Integer[] ids, HttpServletRequest request,
		...
		for (CmsModel bean : beans) {
			log.info("delete CmsModel id={}", bean.getModelId());
	...
	public String priority(Integer[] wids, Integer[] priority,
			Boolean[] isDisabled, Integer defId, HttpServletRequest request,
			ModelMap model) {
		WebErrors errors = validatePriority(wids, priority, isDisabled, defId,
				request);
		if (errors.hasErrors()) {
			return errors.showErrorPage(model);
		}
		manager.updatePriority(wids, priority, isDisabled, defId);
	...
	public void checkId(HttpServletRequest request, HttpServletResponse response) {
		...
		if (StringUtils.isBlank(modelId)) {
			...
			CmsModel model=manager.findById(Integer.parseInt(modelId));
			if(model!=null){
				pass = "false";
			}else{
				pass = "true";
	...
	private WebErrors validatePriority(Integer[] wids, Integer[] priority,
		...
		if (wids.length != priority.length || wids.length != isDisabled.length) {
