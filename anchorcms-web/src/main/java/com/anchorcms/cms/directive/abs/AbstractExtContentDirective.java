package com.anchorcms.cms.directive.abs;
 ...
 * 内容标签基类
 */
public abstract class AbstractExtContentDirective implements
	...
	protected Object getData(Map<String, TemplateModel> params, Environment env,String[] queryParams,String modelPath)
			throws TemplateException {
		...
		if (tagIds != null) {
			...
			if (isPage()) {
				...
				return extContentService.getPageByTagIdsForTag(tagIds, siteIds,
						channelIds, typeIds, excludeId, titleImg, recommend,
						title,attr,orderBy, pageNo, count,queryParams,modelPath);
			} else {
				int first = FrontUtils.getFirst(params);
				return extContentService.getListByTagIdsForTag(tagIds, siteIds,
						channelIds, typeIds, excludeId, titleImg, recommend,
						title,attr, orderBy, first, count,queryParams,modelPath);
			}
		}
		...
		if (topicId != null) {
			...
			if (isPage()) {
				int pageNo = FrontUtils.getPageNo(env);
				return extContentService.getPageByTopicIdForTag(topicId, siteIds,
						channelIds, typeIds, titleImg, recommend, title,attr,
						orderBy, pageNo, count,queryParams,modelPath);
			} else {
				int first = FrontUtils.getFirst(params);
				return extContentService.getListByTopicIdForTag(topicId, siteIds,
						channelIds, typeIds, titleImg, recommend, title,attr,
						orderBy, first, count,queryParams,modelPath);
			}
		}
		...
		if (channelIds != null) {
			...
			if (isPage()) {
				int pageNo = FrontUtils.getPageNo(env);
				return extContentService.getPageByChannelIdsForTag(channelIds,
						typeIds, titleImg, recommend, title,attr, orderBy, option,
						pageNo, count,queryParams,modelPath);
			} else {
				int first = FrontUtils.getFirst(params);
				return extContentService.getListByChannelIdsForTag(channelIds,
						typeIds, titleImg, recommend, title,attr,orderBy, option,
						first, count,queryParams,modelPath);
			}
		}
		...
		if (channelPaths != null) {
			...
			if (pathsToIds) {
				...
				if (channelIds != null) {
					if (isPage()) {
						...
						return extContentService.getPageByChannelIdsForTag(channelIds,
								...
								option,pageNo, count,queryParams,modelPath);
					} else {
						int first = FrontUtils.getFirst(params);
						return extContentService.getListByChannelIdsForTag(channelIds,
								typeIds, titleImg, recommend, title,attr, orderBy,
								option, first, count,queryParams,modelPath);
					}
				} else {
				...
				if (isPage()) {
					int pageNo = FrontUtils.getPageNo(env);
					return extContentService.getPageByChannelPathsForTag(channelPaths,
							siteIds, typeIds, titleImg, recommend, title,attr,
							orderBy, pageNo, count,queryParams,modelPath);
				} else {
					int first = FrontUtils.getFirst(params);
					return extContentService.getListByChannelPathsForTag(channelPaths,
							siteIds, typeIds, titleImg, recommend, title,attr,
							orderBy, first, count,queryParams,modelPath);
				}
			}
		...
		if (isPage()) {
			int pageNo = FrontUtils.getPageNo(env);
			return extContentService.getPageBySiteIdsForTag(siteIds, typeIds,
					titleImg, recommend, title,attr,orderBy, pageNo, count,queryParams,modelPath);
		} else {
			int first = FrontUtils.getFirst(params);
			return extContentService.getListBySiteIdsForTag(siteIds, typeIds,
					titleImg, recommend, title,attr,orderBy, first, count,queryParams,modelPath);
		}
	}
	...
	@Resource
	protected ExtContentService extContentService;
}
