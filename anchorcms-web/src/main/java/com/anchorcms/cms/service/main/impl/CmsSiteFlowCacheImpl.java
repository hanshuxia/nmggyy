...
import com.anchorcms.common.utils.*;
...
import static com.anchorcms.cms.model.assist.CmsSiteAccess.ENGINE_BAIDU;
import static com.anchorcms.cms.model.assist.CmsSiteAccess.ENGINE_GOOGLE;
import static com.anchorcms.cms.model.assist.CmsSiteAccess.ENGINE_YAHOO;
import static com.anchorcms.cms.model.assist.CmsSiteAccess.ENGINE_BING;
import static com.anchorcms.cms.model.assist.CmsSiteAccess.ENGINE_SOGOU;
import static com.anchorcms.cms.model.assist.CmsSiteAccess.ENGINE_SOSO;
import static com.anchorcms.cms.model.assist.CmsSiteAccess.ENGINE_SO;
...
import static com.anchorcms.common.utils.ParseURLKeyword.getKeyword;
...
//	private final String CACHE_KEY_VISITOR_TOTAL="visitorTotal_";
	...
	public Long[] flow(HttpServletRequest request,  String page, String referer) {
		...
		CmsSiteAccess lastAccess = findLastAccess(site.getSiteId());
		...
		lastAccessCache.put(new Element(site.getSiteId(),access));
		//当天第一次访问统计昨日数据
		if(firstVisitToday){
			Thread thread = new StatisticThread(site.getSiteId());
			thread.start();
		}
	...
	public Long[] totalCache(CmsSite site,boolean newVisitor) {
		...
		Element pvCache = pvTotalCache.get(CmsSite.PV_TOTAL+CACHE_KEY_SPLIT+site.getSiteId());
		Element pvWeekCache = pvTotalCache.get(CmsSite.WEEK_PV_TOTAL+CACHE_KEY_SPLIT+site.getSiteId());
		Element pvMonthCache = pvTotalCache.get(CmsSite.MONTH_PV_TOTAL+CACHE_KEY_SPLIT+site.getSiteId());
		...
		Element dayPvCache = dayPvTotalCache.get(CmsSite.DAY_PV_TOTAL+CACHE_KEY_SPLIT+site.getSiteId());
		Long dayPv;
		if (dayPvCache != null) {
		...
		Element visitorCache = visitorTotalCache.get(CmsSite.VISITORS+CACHE_KEY_SPLIT+site.getSiteId());
		Element visitorWeekCache = visitorTotalCache.get(CmsSite.WEEK_VISITORS+CACHE_KEY_SPLIT+site.getSiteId());
		Element visitorMonthCache = visitorTotalCache.get(CmsSite.MONTH_VISITORS+CACHE_KEY_SPLIT+site.getSiteId());
		...
		Long dayVisitor;
		Element dayVisitorCache = dayVisitorTotalCache.get(CmsSite.DAY_VISITORS+CACHE_KEY_SPLIT+site.getSiteId());
		...
		pvTotalCache.put(new Element(CmsSite.PV_TOTAL+CACHE_KEY_SPLIT+site.getSiteId(), pv));
		visitorTotalCache.put(new Element(CmsSite.VISITORS+CACHE_KEY_SPLIT+site.getSiteId(), visitor));
		dayPvTotalCache.put(new Element(CmsSite.DAY_PV_TOTAL+CACHE_KEY_SPLIT+site.getSiteId(), dayPv));
		dayVisitorTotalCache.put(new Element(CmsSite.DAY_VISITORS+CACHE_KEY_SPLIT+site.getSiteId(), dayVisitor));
		pvTotalCache.put(new Element(CmsSite.WEEK_PV_TOTAL+CACHE_KEY_SPLIT+site.getSiteId(), weekPv));
		pvTotalCache.put(new Element(CmsSite.MONTH_PV_TOTAL+CACHE_KEY_SPLIT+site.getSiteId(), monthPv));
		visitorTotalCache.put(new Element(CmsSite.WEEK_VISITORS+CACHE_KEY_SPLIT+site.getSiteId(), weekVisitor));
		visitorTotalCache.put(new Element(CmsSite.MONTH_VISITORS+CACHE_KEY_SPLIT+site.getSiteId(), monthVisitor));
		refreshToDB();
	...
	private void clearFlowInfo(CmsConfig config) {
		...
		for(CmsSite site:siteList){
			cmsSiteMng.updateAttr(site.getSiteId(), attr);
		}
	}
