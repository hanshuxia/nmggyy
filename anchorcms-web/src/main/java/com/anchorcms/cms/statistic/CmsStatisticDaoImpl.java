...
public class CmsStatisticDaoImpl extends HibernateSimpleDao implements
	...
	public List<Object[]> statisticMemberByTarget(Integer target,Date timeBegin,Date timeEnd){
		...
		if(target==STATISTIC_BY_DAY){
			hql="select count(bean.userId),HOUR(bean.registerTime) from CmsUser bean where 1=1 ";  //id改为userId   ---1205 王姝婷
		}else if(target==STATISTIC_BY_MONTH){
			hql="select count(bean.userId),DAY(bean.registerTime) from CmsUser bean  where 1=1  ";
		}else if(target==STATISTIC_BY_YEAR){
			hql="select count(bean.userId),MONTH(bean.registerTime) from CmsUser bean  where 1=1 ";
		}else if(target==STATISTIC_BY_YEARS){
			hql="select count(bean.userId),YEAR(bean.registerTime) from CmsUser bean  where 1=1 ";
		}
		...
		f.append(" and bean.isAdmin=false");  //admin改为isAdmin   ---1205 王姝婷
		if(target==STATISTIC_BY_DAY){
	...
	public long commentStatistic(TimeRange timeRange,
		...
		if (isReplyed != null) {
			if (isReplyed) {
				f.append(" and bean.replyTime is not null");
			} else {
				f.append(" and bean.replyTime is null");
			}
		}
		return (Long) find(f).iterator().next();
	}
