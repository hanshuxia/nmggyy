...
import com.anchorcms.icloud.dao.synergy.*;
...
@Service
@Transactional
public class SynergyCompanyAdminServiceImpl implements SynergyCompanyAdminService {
/**
 * @author dongxuecheng
 ...
 */
    public SCompany save(SCompany bean, Integer channelId, Integer modelId, CmsUser user, Short charge, boolean b) {
      ...
      //  if(user.getCompany() == null){//账号没有企业信息走新增方法
        if(dao.findBytypeAndname(bean)==0){//没有该条记录
       ...
       //     cmsUserDao.update(user);//更新用户表
            ...
            SCompany company = dao.findSCompanyBytypeAndname(bean);
     ...
     *@Desc 企业荣誉列表
     **/
    public Pagination getDiplomaPage(Integer siteId, String companyId, int cpn, int pageSize) {
        return companyDiplomaDao.getPage(siteId, companyId, cpn, pageSize);
    }
    ...
    public SCompanyDiploma saveDiploma(Integer channelId, Integer modelId, CmsUser user,String companyId, SCompanyDiploma diploma) {
        ...
        Date nowDate = new Date(System.currentTimeMillis());
        diploma.setCompanyId(companyId);//企业id
...
/**
 * @author dongxuecheng
 * @Date 2017/2/19 23:08
 * @return
 * @description
 */
    public SCompany findSCompanyById(String Companyid) {
        return dao.findSCompanybyid(Companyid);
    }

    public SCompany findSCompanyBytypeAndname(SCompany bean){return dao.findSCompanyBytypeAndname(bean);}
    ...
    public SCompanyDiploma updateDiploma(Integer channelId, Integer modelId, CmsUser user, Integer diplomaId,
                                         ...
                                         Date deadlineDt, String diplomaPic) {
        Date nowDate = new Date(System.currentTimeMillis());
    ...
    public Pagination getDevicePage(Integer siteId, String companyId, int cpn, int pageSize) {
        return companyDeviceDao.getPage(siteId, companyId, cpn, pageSize);
    }
    ...
    public Content saveDetail(Content c, ContentExt ext, ContentTxt t, String[] picPaths,
                              ...
                              Short charge, boolean forMember,String companyId) {
       ...
       // SCompany company = user.getCompany();
        SCompany company = dao.findSCompanybyid(companyId);
    ...
    @Resource
    private SynergyCompanyAdminDao dao;
    @Resource
    private SCompanyDiplomaAdminDao companyDiplomaDao;
    @Resource
    private SCompanyDeviceAdminDao companyDeviceDao;
    @Resource
