package com.anchorcms.common.constants;

/**
 * @Author 阁楼麻雀
 * @Email netuser.orz@icloud.com
 * @Date 2016-10-31
 * @Desc WEB常量
 */
public class Constants {
    /**
     * 管理路径
     */
    public static final String ADMIN_SUFFIX = "/anchorcms/cmsadmin/index.do";
    /**
     * 首页模板
     */
    public static final String TPLDIR_INDEX = "index";
    /**
     * 栏目页模板
     */
    public static final String TPLDIR_CHANNEL = "channel";
    /**
     * 内容页模板
     */
    public static final String TPLDIR_CONTENT = "content";
    /**
     * 单页模板
     */
    public static final String TPLDIR_ALONE = "alone";
    /**
     * 专题模板
     */
    public static final String TPLDIR_TOPIC = "topic";
    /**
     * 会员中心模板
     */
    public static final String TPLDIR_MEMBER = "member";
    /**
     * 协同制造能力池展示模板
     */
    public static final String TPLDIR_NLCZS = "content";
    /**
     * 备品备件模板
     */
    public static final String TPLDIR_COMMSERVICE = "commservice";
    /**
     * 公共服务模板
     */
    public static final String TPLDIR_SUPPLYCHAIN = "supplychain";
    /**
     * 支付模板
     */
    public static final String TPLDIR_PAYMENT = "payment";
    /**
     * 专用模板
     */
    public static final String TPLDIR_SPECIAL = "special";
    /**
     * 可视化编辑模板
     */
    public static final String TPLDIR_VISUAL = "visual";
    /**
     * 公用模板
     */
    public static final String TPLDIR_COMMON = "common";
    /**
     * 客户端包含模板
     */
    public static final String TPLDIR_CSI = "csi";
    /**
     * 客户端包含用户自定义模板
     */
    public static final String TPLDIR_CSI_CUSTOM = "csi_custom";
    /**
     * 服务器端包含模板
     */
    public static final String TPLDIR_SSI = "ssi";
    /**
     * 标签模板
     */
    public static final String TPLDIR_TAG = "tag";
    /**
     * 评论模板
     */
    public static final String TPLDIR_COMMENT = "comment";
    /**
     * 留言模板
     */
    public static final String TPLDIR_GUESTBOOK = "guestbook";
    /**
     * 站内信模板
     */
    public static final String TPLDIR_MESSAGE = "message";
    /**
     * 列表样式模板
     */
    public static final String TPLDIR_STYLE_LIST = "style_list";
    /**
     * 列表样式模板
     */
    public static final String TPLDIR_STYLE_PAGE = "style_page";

    /**
     * 模板后缀
     */
    public static final String TPL_SUFFIX = ".html";

    /**
     * 前台方言
     */
    public static final String LOCAL_FRONT = "zh_CN";

    /**
     * 上传路径
     */
    public static final String UPLOAD_PATH = "/u/cms/";
    /**
     * 截图路径
     */
    public static final String SNAP_PATH = "/u/cms/snap";
    /**
     * 上传路径
     */
    public static final String LIBRARY_PATH = "/wenku/";
    /**
     *
     */
    public static final String CODE_IMG_PATH = "/codeimg/";
    /**
     * 文库上传文件后缀
     */
    public static final String LIBRARY_SUFFIX = "odt,txt,pdf,wps,et,dps,vsd,pot,pps,rtf,doc,docx,xls,xlsx,ppt,pptx";
    /**
     * 资源路径
     */
    public static final String RES_PATH = "/r/cms";
    /**
     * 模板路径
     */
    public static final String TPL_BASE = "/WEB-INF/tpl/cms";
    /**
     * 全文检索索引路径
     */
    public static final String LUCENE_PATH = "/WEB-INF/lucene";
    /**
     * 列表样式模板路径
     */
    public static final String TPL_STYLE_LIST = "/WEB-INF/tpl/cms_sys_defined/style_list/style_";
    /**
     * 内容分页模板路径
     */
    public static final String TPL_STYLE_PAGE_CONTENT = "/WEB-INF/tpl/cms_sys_defined/style_page/content_";
    /**
     * 列表分页模板路径
     */
    public static final String TPL_STYLE_PAGE_CHANNEL = "/WEB-INF/tpl/cms_sys_defined/style_page/channel_";
    /**
     * 页面禁止访问
     */
    public static final String ERROR_403 = "error/403";
    /**
     * 数据库备份路径
     */
    public static final String BACKUP_PATH = "/WEB-INF/backup";
    /**
     * 数据库备份文本前缀
     */
    public static String ONESQL_PREFIX="ANCHORCMS_BACKUP_";
    /**
     * 防火墙配置文件
     */
    public static String FIREWALL_CONFIGPATH = "/WEB-INF/classes/manager/firewall.properties";
    /**
     * 类--错误国际化信息配置
     */
    public static String CLASS_ERROR_CODE = "/WEB-INF/classes-error.properties";
    /**
     * 配置文件
     */
    public static String ANCHORCMS_CONFIG = "/WEB-INF/anchorcms.properties";
    /**
     * 插件路径
     */
    public static final String PLUG_PATH = "/WEB-INF/plug/";
    /**
     * 标签模板路径
     */
    public static final String DIRECTIVE_TPL_PATH = "/WEB-INF/directive/";
    /**
     * 后台资源路径
     */
    public static final String RES_BACK_PATH = "/res";
    /**
     * 模板路径
     */
    public static final String TPL_BACK_BASE = "/WEB-INF/acnchorcms_sys";


    /**
     * 路径分隔符
     */
    public static final String SPT = "/";
    /**
     * 索引页
     */
    public static final String INDEX = "index";
    /**
     * 手机索引页
     */
    public static final String INDEX_MOBILE = "indexMobile";
    /**
     * 索引页
     */
    public static final String INDEX_HTML = "/index.html";
    /**
     * 手机索引页
     */
    public static final String INDEX_HTML_MOBILE = "/indexMobile.html";
    /**
     * 默认模板
     */
    public static final String DEFAULT = "default";
    /**
     * UTF-8编码
     */
    public static final String UTF8 = "UTF-8";
    /**
     * 提示信息
     */
    public static final String MESSAGE = "message";
    /**
     * cookie中的JSESSIONID名称
     */
    public static final String JSESSION_COOKIE = "JSESSIONID";
    /**
     * url中的jsessionid名称
     */
    public static final String JSESSION_URL = "jsessionid";
    /**
     * HTTP POST请求
     */
    public static final String POST = "POST";
    /**
     * HTTP GET请求
     */
    public static final String GET = "GET";
    public static final String WEIXIN_APPKEY="wxAppkey";
    public static final String WEIXIN_APPSECRET="wxAppSecret";
    public static final int DISTRIBUTE_THREAD_COUNT=10;

    public static final String EXT_NAME_IN_REQUEST = "jxd_ext";
    public static final String EXT_PATH_IN_REQUEST = "jxd_ext_path";
}
